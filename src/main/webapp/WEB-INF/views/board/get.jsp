	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				aria-hidden="true">&times;</button><!-- x &nbsp; -->
			...
			<div class="modal-body">
				...
				<div class="form-group">
					<label>Replyer</label>
					<input class="form-control" name="replyer" placeholder="replyer" readonly>
				</div>
			</div>
			<!-- 버튼 모음 (수정, 삭제, 등록, 닫기...) -->
...
var replyService = (function() {
	...
	function getList(param, callback, error) {
		...
		var page = param.page || 1; // 페이지 정보가 없을수도 있으니까
		// 페이지정보가 5페이지 => 그대로 쓰고
	...
	// replyservice() 함수를 부르고 나서 그 결과값에 객체가들어오는데
	// 그안에 add 를 가져오면 댓글 등록기능 함수를 사용할수 있다.
...
$(document).ready(function() {
	var bnoValue = '<c:out value="${board.bno}"/>';
	//우리가 li 태그를 동적으로 추가한다는말은
	//ul 태그를 찾아서 그 태그의 자손으로 붙여 주겠다.
	//ul.html(dddddd) : ul태그의 안의 html 코드를 수정
	var replyUL = $(".chat");
	
	//동적으로 댓글을 만들어서 붙여주는 함수
	//요청이 성공했으면 그때 댓글을 만들어서 붙여주면 된다.
	//요청이 성공했으면 그때 댓글을 만든다 ===> callback 함수에서 처리하면된다.
	showList(1);
	...
	function showList(page) {
		...
		replyService.getList({
			...
			if(page == -1){
				// 댓글 개수가 7개일때 페이지 몇개? (1페이지에 10개라고 하면) 페이지 개수 1개
	...
	// 동적으로 생성될 요소의 부모 또는 형제 요소(이미 존재하는 요소)에다가 달아준 다음
	// 나중에 이벤트의 대상을 변경해준다.
